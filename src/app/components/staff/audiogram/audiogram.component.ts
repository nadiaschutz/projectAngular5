        ...
        this.route.params.subscribe(params => {
            ...
            this.audiogramService.getAllEOC(this.eocId).subscribe(bundle => {
                ...
                if (bundle && bundle['entry'] && Array.isArray(bundle['entry']) && bundle['entry'].length > 0) {
                    ...
                    const observation = JSON.parse(JSON.stringify(this.observations));
                    const sortArray = observation.sort((a: any, b: any) => {
                        const dateA: any = new Date(a.issued);
                        const dateB: any = new Date(b.issued);
                        return dateB - dateA;
                    });

                    this.observations = sortArray;
